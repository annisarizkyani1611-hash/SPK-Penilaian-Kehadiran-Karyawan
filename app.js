...
// ROUTE KARYAWAN (FIX SORTING)
app.get('/karyawan', auth, async (req, res) => {
    // PENTING: Kriteria harus diurutkan Kode ASC agar C1 selalu pertama
...
app.post('/save_karyawan', auth, async (req, res) => {
    ...
    if(nilai) {
        for(let [cid, val] of Object.entries(nilai)) {
            ...
            // Cek dulu baru update (Metode Paling Aman)
            const cek = await query("SELECT * FROM nilai WHERE id_karyawan=? AND id_kriteria=?", [kid, cid]);
            if(cek.length > 0) await query("UPDATE nilai SET nilai=? WHERE id_karyawan=? AND id_kriteria=?", [v, kid, cid]);
            else await query("INSERT INTO nilai (id_karyawan, id_kriteria, nilai) VALUES (?,?,?)", [kid, cid, v]);
        }
    }
